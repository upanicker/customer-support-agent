...
load_dotenv()
# Read the input knowledge only once before embedding and upserting it.
...
# Create the index only for a first-time setup; existing indexes are reused.
if INDEX_NAME not in [item["name"] for item in pc.list_indexes()]:
    pc.create_index(name=INDEX_NAME, dimension=1536, metric="cosine", spec=ServerlessSpec(cloud="aws", region="us-east-1"))
    while not pc.describe_index(INDEX_NAME).status["ready"]:
        time.sleep(2)

# Existing indexes may use a smaller dimension, so inspect it before embedding.
index_description = pc.describe_index(INDEX_NAME)
INDEX_DIMENSION = getattr(index_description, "dimension", None) or index_description["dimension"]

client = OpenAI(api_key=os.environ["OPENAI_API_KEY"])
# Request embeddings that exactly match the Pinecone index dimension.
embeddings = client.embeddings.create(
    model="text-embedding-3-small",
    input=[doc["content"] for doc in DOCS],
    dimensions=INDEX_DIMENSION,
).data
# Keep article fields as metadata so they can be displayed with each retrieval result.
